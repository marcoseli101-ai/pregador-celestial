    ...
    >
      ...
      {slide.type === "title" || slide.type === "subtitle" || slide.type === "conclusion" ? (
        <div className="text-center space-y-4 max-w-[85%]">
          <h2 className="font-bold leading-tight" style={{ color: isDark ? "#fff" : colors.accent, fontSize: "clamp(1.4rem, 4vw, 2.5rem)" }}>
            {slide.heading}
          </h2>
          {slide.bullets?.map((b, i) => (
            <p key={i} className="text-base md:text-lg" style={{ color: isDark ? "rgba(255,255,255,0.85)" : "#333" }}>{b}</p>
          ))}
          {slide.verse && <p className="italic text-sm md:text-base mt-4" style={{ color: colors.accent }}>{slide.verse}</p>}
        </div>
      ) : (
        <div className="w-full max-w-[90%] space-y-4">
          <h3 className="font-bold text-lg md:text-2xl mb-4" style={{ color: colors.accent }}>{slide.heading}</h3>
          ...
          {slide.verse && (
            <blockquote className="border-l-4 pl-4 italic text-sm md:text-base mt-4" style={{ borderColor: colors.accent, color: "#555" }}>
              {slide.verse}
            </blockquote>
          )}
        </div>
...
export function SlideGeneratorModal({ content, open, onClose }: SlideGeneratorModalProps) {
  ...
  const downloadPDF = useCallback(() => {
    ...
    slideData.slides.forEach((slide, i) => {
      ...
      const ar = parseInt(colors.accent.slice(1, 3), 16), ag = parseInt(colors.accent.slice(3, 5), 16), ab = parseInt(colors.accent.slice(5, 7), 16);
  ...
  const downloadPPTX = useCallback(async () => {
    ...
    slideData.slides.forEach((slide) => {
      ...
      pSlide.addShape(pptx.ShapeType.rect, { x: 0, y: 0, w: "100%", h: 0.08, fill: { color: colors.accent.replace("#", "") } });

      if (isDark) {
        pSlide.addText(slide.heading, { x: 0.5, y: 1.5, w: "90%", h: 1.5, fontSize: 36, bold: true, color: "FFFFFF", align: "center", valign: "middle" });
        const bulletText = (slide.bullets || []).map(b => ({ text: b, options: { fontSize: 18, color: "E6E6E6", bullet: false, breakLine: true } }));
        if (bulletText.length) pSlide.addText(bulletText, { x: 0.8, y: 3.2, w: "85%", h: 2.5, align: "center", valign: "top" });
      } else {
        pSlide.addText(slide.heading, { x: 0.5, y: 0.3, w: "90%", h: 1, fontSize: 28, bold: true, color: colors.accent.replace("#", ""), valign: "top" });
        const bulletText = (slide.bullets || []).map(b => ({ text: b, options: { fontSize: 16, color: "333333", bullet: { type: "bullet" as const }, breakLine: true } }));
        if (bulletText.length) pSlide.addText(bulletText, { x: 0.8, y: 1.5, w: "85%", h: 3.5, valign: "top" });
      }

      if (slide.verse) {
        pSlide.addText(slide.verse, { x: 0.8, y: isDark ? 5 : 4.5, w: "85%", h: 0.7, fontSize: 13, italic: true, color: colors.accent.replace("#", ""), align: isDark ? "center" : "left" });
      }
    });
      ...
      <div className="relative w-full max-w-4xl max-h-[90vh] bg-card rounded-2xl shadow-2xl flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-auto p-6">
          {loading ? (
            ...
            <SlidePreview slide={slideData.slides[currentSlide]} index={currentSlide} total={slideData.slides.length} />
          ) : null}
        </div>
